...
// New byte-based transport implementation (Phase 3)
bool ZMQTransport::SendBytes(std::unique_ptr<std::vector<uint8_t>>& data)
{
  ...
  try {
    ...
    // If memory pool is enabled and send was successful, potentially reuse buffer
    // (Future enhancement: actual buffer pooling for byte vectors)
    ...
    const std::unique_ptr<std::vector<std::unique_ptr<EventData>>> &events)
{
  ...
  // Compatibility mode: Use temporary local serializer and sequence number
  static uint64_t sequence_counter = 0;
  Serializer temp_serializer;
  temp_serializer.EnableCompression(fCompressionEnabled);
  temp_serializer.EnableChecksum(fChecksumEnabled);
  auto encoded_data = temp_serializer.Encode(events, ++sequence_counter);
...
ZMQTransport::Receive()
{
  ...
  try {
    ...
    // Compatibility mode: Use temporary local serializer to decode the data
    Serializer temp_serializer;
    temp_serializer.EnableCompression(fCompressionEnabled);
    temp_serializer.EnableChecksum(fChecksumEnabled);
    auto decoded_result = temp_serializer.Decode(received_data);
    ...
    const std::unique_ptr<std::vector<std::unique_ptr<MinimalEventData>>> &events)
{
  ...
  // Compatibility mode: Use temporary local serializer and sequence number
  static uint64_t minimal_sequence_counter = 0;
  Serializer temp_serializer;
  temp_serializer.EnableCompression(fCompressionEnabled);
  temp_serializer.EnableChecksum(fChecksumEnabled);
  auto encoded_data = temp_serializer.Encode(events, ++minimal_sequence_counter);
...
ZMQTransport::ReceiveMinimal()
{
  ...
  try {
    ...
    // Compatibility mode: Use temporary local serializer to decode the MinimalEventData
    Serializer temp_serializer;
    temp_serializer.EnableCompression(fCompressionEnabled);
    temp_serializer.EnableChecksum(fChecksumEnabled);
    return temp_serializer.DecodeMinimalEventData(received_data);
    ...
    if (received_data->size() >= sizeof(BinaryDataHeader)) {
      ...
      if (header.magic_number == BINARY_DATA_MAGIC_NUMBER) {
        if (header.format_version == FORMAT_VERSION_MINIMAL_EVENTDATA) {
          // Decode as MinimalEventData using temporary serializer
          Serializer temp_serializer;
          temp_serializer.EnableCompression(fCompressionEnabled);
          temp_serializer.EnableChecksum(fChecksumEnabled);
          auto [events, seq] = temp_serializer.DecodeMinimalEventData(received_data);
          return std::make_pair(VariantType{std::move(events)}, seq);
        } else if (header.format_version == FORMAT_VERSION_EVENTDATA) {
          // Decode as EventData using temporary serializer
          Serializer temp_serializer;
          temp_serializer.EnableCompression(fCompressionEnabled);
          temp_serializer.EnableChecksum(fChecksumEnabled);
          auto [events, seq] = temp_serializer.Decode(received_data);
          return std::make_pair(VariantType{std::move(events)}, seq);
        }
      }
    }
    
    // Fallback to EventData if format detection fails using temporary serializer
    Serializer temp_serializer;
    temp_serializer.EnableCompression(fCompressionEnabled);
    temp_serializer.EnableChecksum(fChecksumEnabled);
    auto [events, seq] = temp_serializer.Decode(received_data);
...
void ZMQTransport::EnableCompression(bool enable)
{
  ...
  // Note: Settings now apply only to legacy methods' temporary serializers
}
