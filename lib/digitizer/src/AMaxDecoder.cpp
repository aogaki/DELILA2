#include "../include/AMaxDecoder.hpp"
...
#include "../include/AMaxConstants.hpp"

using namespace DELILA::Digitizer::AMaxConstants;
...
// Constants are now defined in AMaxConstants.hpp

// ============================================================================
// Constructor/Destructor
// ============================================================================

AMaxDecoder::AMaxDecoder(uint32_t nThreads)
{
  if (nThreads < 1) {
    nThreads = 1;
  }
  ...
  for (uint32_t i = 0; i < nThreads; ++i) {
    fDecodeThreads.emplace_back(&AMaxDecoder::DecodeThread, this);
  }
}

AMaxDecoder::~AMaxDecoder()
{
...
void AMaxDecoder::DecodeData(std::unique_ptr<RawData_t> rawData)
{
  if (fDumpFlag) {
    DumpRawData(*rawData);
  }
  ...
  // TODO: Update validation for AMax format
...
void AMaxDecoder::DumpRawData(const RawData_t &rawData) const
{
...
bool AMaxDecoder::ValidateDataHeader(uint64_t headerWord, size_t dataSize)
{
  // Check header type
...
void AMaxDecoder::ProcessEventData(
...
std::unique_ptr<EventData> AMaxDecoder::DecodeEventPair(
    const std::vector<uint8_t>::iterator &dataStart, size_t &wordIndex)
{
  // TODO: Update event decoding for AMax format
  // This is placeholder logic based on PSD2Decoder structure
  ...
  // Read third word
  uint64_t thirdWord = 0;
  std::memcpy(&thirdWord, &(*(dataStart + wordIndex * kWordSize)),
              sizeof(uint64_t));
  wordIndex++;

  // Read fourth word
  uint64_t fourthWord = 0;
  std::memcpy(&fourthWord, &(*(dataStart + wordIndex * kWordSize)),
              sizeof(uint64_t));
  wordIndex++;
...
void AMaxDecoder::DecodeFirstWord(uint64_t word, EventData &eventData) const
{
  // Extract channel
...
void AMaxDecoder::DecodeSecondWord(uint64_t word, EventData &eventData,
...
void AMaxDecoder::DecodeWaveformData(
    const std::vector<uint8_t>::iterator &dataStart, size_t &wordIndex,
    EventData &eventData)
{
  ...
  // Decode waveform header (if needed for probe types, time resolution, etc.)
  ...
  // AMax format: Each 64-bit word contains four 16-bit samples
  // Sample layout: [0:15], [16:31], [32:47], [48:63]
  for (size_t j = 0; j < nWordsWaveform; j++) {
    ...
    // Extract four 16-bit samples from the 64-bit word
    uint16_t sample0 = static_cast<uint16_t>(waveformWord & 0xFFFF);
    uint16_t sample1 = static_cast<uint16_t>((waveformWord >> 16) & 0xFFFF);
    uint16_t sample2 = static_cast<uint16_t>((waveformWord >> 32) & 0xFFFF);
    uint16_t sample3 = static_cast<uint16_t>((waveformWord >> 48) & 0xFFFF);

    // Decode each sample
    size_t baseIndex = j * 4;
    DecodeWaveformPoint(sample0, baseIndex + 0, config, eventData);
    DecodeWaveformPoint(sample1, baseIndex + 1, config, eventData);
    DecodeWaveformPoint(sample2, baseIndex + 2, config, eventData);
    DecodeWaveformPoint(sample3, baseIndex + 3, config, eventData);
  }
}

void AMaxDecoder::DecodeWaveformHeader(uint64_t header,
                                       EventData &eventData) const
{
  // TODO: Update waveform header decoding for AMax format

  // Extract time resolution
  auto timeResolution = (header >> 44) & 0x3;  // TODO: Update for AMax format
...
AMaxDecoder::WaveformConfig AMaxDecoder::ExtractWaveformConfig(
    uint64_t header) const
{
  // TODO: Update waveform configuration extraction for AMax format
...
uint32_t AMaxDecoder::GetMultiplicationFactor(uint32_t encodedValue) const
{
  switch (encodedValue) {
    case 0:
      return 1;
...
void AMaxDecoder::DecodeWaveformPoint(uint16_t sample, size_t dataIndex,
                                      const WaveformConfig &config,
                                      EventData &eventData) const
{
  // AMax format: 16-bit sample with analog probe in lower 16 bits
  // For now, store the raw 16-bit sample in analogProbe1
  // Digital probes and analogProbe2 are not used in AMax format

  uint32_t analogRaw = sample;  // 16-bit sample

  if (config.ap1IsSigned) {
    // Sign extend from 16-bit to 32-bit
    if (analogRaw & 0x8000) analogRaw |= 0xFFFF0000;
    eventData.analogProbe1[dataIndex] =
        static_cast<int32_t>(analogRaw) * config.ap1MulFactor;
  } else {
    eventData.analogProbe1[dataIndex] = analogRaw * config.ap1MulFactor;
  }

  // AMax doesn't use analog probe 2 or digital probes in the same way as PSD2
  eventData.analogProbe2[dataIndex] = 0;
  eventData.digitalProbe1[dataIndex] = 0;
  eventData.digitalProbe2[dataIndex] = 0;
  eventData.digitalProbe3[dataIndex] = 0;
  eventData.digitalProbe4[dataIndex] = 0;
}
...
DataType AMaxDecoder::AddData(std::unique_ptr<RawData_t> rawData)
{
...
DataType AMaxDecoder::CheckDataType(std::unique_ptr<RawData_t> &rawData)
{
  if (rawData->size < Validation::kMinimumDataSize) {
    std::cout << "Unknown data type due to insufficient size" << std::endl;
    return DataType::Unknown;
  } else if (rawData->size == Validation::kStopSignalSize) {
    if (CheckStop(rawData)) {
      return DataType::Stop;
    }
  } else if (rawData->size == Validation::kStartSignalSize) {
...
bool AMaxDecoder::CheckStop(std::unique_ptr<RawData_t> &rawData)
{
  uint64_t buf = 0;
  ...
  auto firstCondition =
      ((buf >> StartStop::kSignalTypeShift) & StartStop::kSignalTypeMask) == StartStop::kStopFirstWordType &&
      ((buf >> StartStop::kSignalSubTypeShift) & StartStop::kSignalTypeMask) == StartStop::kStopFirstWordSubType;

  // The second word bit[56:63] = 0x0
  std::memcpy(&buf, &(*(rawData->data.begin() + 8)), sizeof(uint64_t));
  auto secondCondition = ((buf >> StartStop::kSignalSubTypeShift) & StartStop::kSignalTypeMask) == StartStop::kStopSecondWordType;

  // The third word bit[56:63] = 0x1
  std::memcpy(&buf, &(*(rawData->data.begin() + 16)), sizeof(uint64_t));
  auto thirdCondition = ((buf >> StartStop::kSignalSubTypeShift) & StartStop::kSignalTypeMask) == StartStop::kStopThirdWordType;

  if (firstCondition && secondCondition && thirdCondition) {
    // The third word bit[0:31] = dead time
    // auto deadTime = static_cast<uint32_t>(buf & StartStop::kDeadTimeMask);
    // std::cout << "Dead time: " << deadTime * StartStop::kDeadTimeScale << " ns" << std::endl;
    return true;
  }
...
bool AMaxDecoder::CheckStart(std::unique_ptr<RawData_t> &rawData)
{
  uint64_t buf = 0;
  // The first word bit[60:63] = 0x3
  // The first word bit[56:59] = 0x0
  // The first word bit[32:39] = 0x3
  // The first word bit[0:31] = 0x4
  std::memcpy(&buf, &(*(rawData->data.begin())), sizeof(uint64_t));
  auto firstCondition =
      ((buf >> StartStop::kSignalTypeShift) & StartStop::kSignalTypeMask) == StartStop::kStartFirstWordType &&
      ((buf >> StartStop::kSignalSubTypeShift) & StartStop::kSignalTypeMask) == StartStop::kStartFirstWordSubType &&
      ((buf >> StartStop::kSignalExtra1Shift) & StartStop::kSignalExtra1Mask) == StartStop::kStartFirstWordExtra1 &&
      ((buf >> StartStop::kSignalExtra2Shift) & StartStop::kSignalExtra2Mask) == StartStop::kStartFirstWordExtra2;

  // The second word bit[56:63] = 0x2
  std::memcpy(&buf, &(*(rawData->data.begin() + 8)), sizeof(uint64_t));
  auto secondCondition = ((buf >> StartStop::kSignalSubTypeShift) & StartStop::kSignalTypeMask) == StartStop::kStartSecondWordType;

  // The third word bit[56:63] = 0x1
  std::memcpy(&buf, &(*(rawData->data.begin() + 16)), sizeof(uint64_t));
  auto thirdCondition = ((buf >> StartStop::kSignalSubTypeShift) & StartStop::kSignalTypeMask) == StartStop::kStartThirdWordType;

  // The fourth word bit[56:63] = 0x1
  std::memcpy(&buf, &(*(rawData->data.begin() + 24)), sizeof(uint64_t));
  auto fourthCondition = ((buf >> StartStop::kSignalSubTypeShift) & StartStop::kSignalTypeMask) == StartStop::kStartFourthWordType;
